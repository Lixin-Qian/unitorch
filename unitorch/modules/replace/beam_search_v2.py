...
@replace(transformers.generation_logits_process.NoRepeatNGramLogitsProcessor)
class NoRepeatNGramLogitsProcessorV2(transformers.generation_logits_process.LogitsProcessor):
    r"""
    Args:
        ngram_size (:obj:`int`):
...
@replace(transformers.generation_beam_search.BeamSearchScorer)
class BeamSearchScorerV2(transformers.generation_beam_search.BeamSearchScorer):
    def __init__(
        self,
        batch_size: int,
        num_beams: int,
        ...
        self.beams_offset = (torch.arange(0, batch_size) * num_beams // self.num_beam_groups).unsqueeze(1).to(device)
    ...
    def process(
        ...
        if self.use_reorder_cache_v2 and eos_effective_idx.numel() > 0:
            ...
            unfin_offset = np.array(list(accumulate(map(int, self._done))))[np.array(list(map(int, self._done))) == 0]
            for i in range(eos_effective_idx.size(0)):
                ...
                if not self._done[batch_idx]:
                    self._beam_hyps[batch_idx.item()].add(input_clone[i], eos_score.item())
        ...
        _, active_hypos = torch.topk(active_mask, k=self.group_size, dim=1, largest=False)
