    ...
    for k, input_buffer_k in attn_cache.items():
        if input_buffer_k is not None:
            if "prefix_" in k:
                ...
                attn_cache[k] = input_buffer_k if beam_idx is None else input_buffer_k.index_select(0, beam_idx)
    return attn_cache
...
class PrefixSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.gradient_checkpointing and self.training:
            past_prefix_key_layer = None
            past_prefix_value_layer = None
            past_key_layer = None
            past_value_layer = None
        else:
        ...
        if past_prefix_key_layer is not None:
            ...
            prefix_attention_scores = prefix_attention_scores.reshape(-1, *prefix_attention_scores.size()[2:])
            ...
            attention_scores = torch.cat((prefix_attention_scores, attention_scores), dim=-1)
            ...
            prefix_context_layer = torch.einsum(
                "bxhtd,bhds->bxhts",
                prefix_attention_probs.view(prefix_size[0], -1, *prefix_attention_probs.size()[1:]),
                past_prefix_value_layer,
            )
            prefix_context_layer = prefix_context_layer.reshape(-1, *prefix_context_layer.size()[2:])
...
class PrefixEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList([PrefixLayer(config) for _ in range(config.num_hidden_layers)])
        self.gradient_checkpointing = config.gradient_checkpointing
...
class PrefixTextModel(PrefixPreTrainedModel):
    def __init__(self, config, add_pooling_layer=False):
        ...
        if self.enable_relative_position:
            self.rel_pos_bias = nn.Linear(config.rel_pos_bins, config.num_attention_heads, bias=False)
    ...
    def forward(
        ...
        if self.enable_relative_position and rel_pos is not None:
            rel_pos = F.one_hot(rel_pos, num_classes=self.config.rel_pos_bins).to(self.rel_pos_bias.weight)
            rel_pos = self.rel_pos_bias(rel_pos).permute(0, 3, 1, 2)
        elif self.enable_relative_position:
            rel_pos_mat = position_ids.unsqueeze(-2) - position_ids.unsqueeze(-1)
            rel_pos = _relative_position_bucket(rel_pos_mat, num_buckets=32, max_distance=128)
            rel_pos = F.one_hot(rel_pos, num_classes=self.config.rel_pos_bins).to(self.rel_pos_bias.weight)
            rel_pos = self.rel_pos_bias(rel_pos).permute(0, 3, 1, 2)
...
class PrefixPixelModel(PrefixPreTrainedModel):
    ...
    def forward(
        ...
        pooled_output = self.pooler(sequence_output[:, pixel_seq_length:]) if self.pooler is not None else None
