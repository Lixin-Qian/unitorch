...
class OptimScheduler(object):
    def __init__(self, OptimScheduler):
        self.OptimScheduler = OptimScheduler

    def from_checkpoint(
        self,
        ckpt_dir,
        weight_name="pytorch_OptimScheduler.bin",
        is_optim=True,
    ):
        if self.OptimScheduler is None:
            return
        weight_path = os.path.join(ckpt_dir, weight_name)
        if not os.path.exists(weight_path):
            return self.OptimScheduler
        if is_optim:
            state_dict = [torch.load(weight_path, map_location="cpu")]
        else:
            state_dict = torch.load(weight_path, map_location="cpu")
        self.OptimScheduler.load_state_dict(state_dict)
        return self.OptimScheduler
    ...
    def save_checkpoint(
        ...
        if self.OptimScheduler is None:
            return
        weight_path = os.path.join(ckpt_dir, weight_name)
        state_dict = self.OptimScheduler.state_dict()
        torch.save(state_dict, weight_path)


@register_task("core/deepspeed/supervised_task")
class DeepspeedTask(object):
    def __init__(
        self,
        __config__=None,
        model=None,
    ...
    @classmethod
    @add_default_section_for_init("core/deepspeed/supervised_task")
    def from_core_configure(cls, config, **kwargs):
        ...
        config.set_default_section("core/deepspeed/supervised_task")
    ...
    @add_default_section_for_function("core/deepspeed/supervised_task")
    def train(
        self,
        ...
        """
        if os.path.exists(from_ckpt_dir):
            optim = OptimScheduler(optim).from_checkpoint(
                from_ckpt_dir, weight_name="pytorch_optim.bin",
            )
        ...
        if scheduler and os.path.exists(to_ckpt_dir):
            scheduler = OptimScheduler(scheduler).from_checkpoint(
                to_ckpt_dir,
                weight_name="pytorch_scheduler_latest.bin",
                is_optim=False,
            )
